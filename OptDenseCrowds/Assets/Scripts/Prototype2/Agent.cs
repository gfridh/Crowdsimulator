...
public class Agent : MonoBehaviour {
	...
	internal void calculatePreferredVelocityMap(ref MapGen.map map) {
		if ((transform.position - map.allNodes [path [pathIndex]].getTargetPoint(transform.position)).magnitude < map.allNodes[path[pathIndex]].getThreshold() || (Grid.instance.skipNodeIfSeeNext && canSeeNext(ref map, 1))) { // || 
			...
			if (pathIndex >= path.Count) {
				...
				Vector3 nextPref = ((map.allNodes [path [pathIndex]].getTargetPoint(transform.position)) - transform.position).normalized;
				if (Vector3.Angle (previousForward, nextPref) > 30.0f) {
					
					preferredVelocity = Vector3.RotateTowards (transform.forward, nextPref, 3.0f * Mathf.PI / 180.0f, 15.0f).normalized;
				}
			}
			...
			Vector3 previousForward = transform.forward;
			Vector3 nextPref = ((map.allNodes [path [pathIndex]].getTargetPoint(transform.position)) - transform.position).normalized;
			if (Vector3.Angle (previousForward, nextPref) > 20.0f) {
				
				preferredVelocity = Vector3.RotateTowards (transform.forward, nextPref, 1.0f * Mathf.PI / 180.0f,  15.0f).normalized;
			} else {
	...
	internal void changePosition(ref MapGen.map map) {
		...
		float realSpeed = Vector3.Distance (transform.position, prevPos) / Grid.instance.dt;
		if (animator != null) {
			if (realSpeed < 0.05f) {
				animator.speed = 0;
			} else {
				animator.speed = (realSpeed / (Grid.instance.agentMaxSpeed * Time.deltaTime)) * Time.deltaTime;
			}
		}
	}
