...
public class MapGen : MonoBehaviour {
	...
	public static float DEFAULT_THRESHOLD = 4.0f;
	...
	//	bool[,] notFree = new bool[(int)(xMinMax.y - xMinMax.x), (int)(zMinMax.y - zMinMax.x)];
		...
		foreach(CustomNode c in Object.FindObjectsOfType<CustomNode> ()) {
			...
			if (r != null) {
				r.enabled = visibleMap ? true : false;
			}
		...
		for(int i = 0; i < nodes; ++i) {
			...
			while(true) {
				...
				if ((q-s).magnitude > (news-s).magnitude ) {
					break;
				}
			...
			if ((s - getBorderDistance (s, xMinMax, zMinMax)).magnitude < 3) {
				continue;
			}
			map.Add (s);
		...
		List<List<List<int>>> shortestPaths = getShortestPaths (ref dist);
	...
	private List<List<List<int>>> getShortestPaths(ref List<List<float>> dist) {
